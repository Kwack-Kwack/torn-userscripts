...
// @description Show tornstats spies on faction wall page || Edited for PDA by Kwack [2190604]
...
let apiKey;
if("###PDA-APIKEY###".charAt(0) === "#") {
	apiKey = localStorage["finally.torn.api"] || null;
} else apiKey = "###PDA-APIKEY###"
...
function checkApiKey(key, cb) {
	PDA_httpGet(`https://www.tornstats.com/api/v1/${key}`).then(r => {
		if (r.status == 429){
	...
	}).catch(e => cb(`Couldn't check, error ${e.toString()}`));
}
...
function loadTSFactions(id) {
	...
	PDA_httpGet(`https://www.tornstats.com/api/v2/${apiKey}/spy/faction/${id}`).then(r => {
		let j = JSONparse(r.responseText);
