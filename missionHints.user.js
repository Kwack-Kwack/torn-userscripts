...
// @version      0.1
// @description  A userscript that adapts the TT mission hints feature into to TornPDA
// @author       Kwack [2190604]
// @match        *://www.torn.com/loader.php?sid=missions
// ==/UserScript==

//See https://github.com/Mephiles/torntools_extension/blob/master/extension/scripts/features/mission-hints/ttMissionHints.js
...
Object.defineProperty(Document.prototype, "newElement", {
    value(options = {}) {
		if (typeof options === "string") {
			...
			if (options.class) {
				if (Array.isArray(options.class)) newElement.classList.add(...options.class.filter((name) => !!name));
				else newElement.classList.add(options.class.trim());
			}
...
let style = `.tt-mission-information {
	margin-top: 16px;
}

.tt-mission-information > span {
	display: block;
}
...
async function pageReady() {
    ...
    return new Promise(resolve => {
        let checker = setInterval(() => {
            if(document.querySelectorAll("div[id^=mission]").length > 1) {
                console.log("Page loaded, missions hint script starting...", Array.from(document.querySelectorAll("div[id^=mission]")));
                console.log()
                clearInterval(checker);
                resolve()
            } else {
                console.error("No mission container found!", Array.from(document.querySelectorAll("div[id^=mission]")));
...
await pageReady();
await new Promise(r => setTimeout(r, 500)) //Unfortunately script fires the second it detects a mission, and it is impossible to await all missions without knowing how many missions there should be. Waiting 0.5s after the first mission loads should be enough.

for(let missionElem of Array.from(document.querySelectorAll(".giver-cont-wrap > div[id^=mission]"))) {
    const title = missionElem.querySelector(".title-black").childNodes[0].wholeText.replace(/\n/g, "").trim();
    const key = title.toLowerCase().replace(/[\s-]/g, "_").replace(/[:!,]/g, "");
    let task, hint;
    if(key in MISSION_HINTS) {
        ...
        task = "Either this script needs to be updated, or contact me @duckyblair#0001 on discord"
    }

    let children = [document.newElement({ type: "h6", class: "tt-mission-title", text: "Mission information" }), document.newElement({ type: "span", children: [document.newElement({ type: "b", text: "Task: "}), task] })];

    if(hint) {
        children.push(document.newElement("br"), document.newElement({ type: "span", children: [document.newElement({ type: "b", text: "Hint: " }), hint] }));
    }

    missionElem.querySelector(".max-height-fix").appendChild(document.newElement({ type: "div", class: "tt-mission-information", children }))
}
